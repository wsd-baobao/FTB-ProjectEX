...
public class OfflineKnowledgeManager extends SavedData {
    private static final String DATA_NAME = ProjectEX.MOD_ID + ":offline_emc";
    ...
    public static OfflineKnowledgeManager getInstance() {
        if (instance == null) {
            ...
            instance = overworld.getDataStorage().computeIfAbsent(
                    OfflineKnowledgeManager::new,
                     DATA_NAME);
        }
        return instance;
    }

    private static OfflineKnowledgeManager loadData(CompoundTag tag) {
    ...
    private IKnowledgeProvider getOnlineProvider(Player player) {
        try {
            return player.getCapability(ProjectEAPI.KNOWLEDGE_CAPABILITY).orElseThrow(NoSuchElementException::new);
    ...
    public static class Listener {
        ...
        public static void onPlayerConnect(PlayerEvent.PlayerLoggedInEvent event) {
            if (!event.getPlayer().level.isClientSide()) {
                event.getPlayer().getCapability(ProjectEAPI.KNOWLEDGE_CAPABILITY)
